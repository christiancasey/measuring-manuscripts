...
The same pattern runs through the instructor's Egyptian and Coptic work. "Hieratic is messier than hieroglyphic" only becomes a research result once "messier" is fixed to a specific measure—say, the spread of shapes for one sign across a corpus. Until then it's an impression.

> 🔧 TO BUILD: drop in the instructor's own worked operationalization from the hieratic sign-variation work—the abstract claim, the chosen measure, and one number that came out of it.
...
- **A question no available data can answer.** The question is fine; the data to answer it doesn't exist, or you can't reach it in two weeks. Check that the data is in hand *before* you fall in love with the question.
