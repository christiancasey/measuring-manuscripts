...
:class: tip
Everything today is built on a small starter website in [**project-template/**](https://github.com/christiancasey/measuring-manuscripts-project-template). It is already a working [GitHub Pages site](https://christiancasey.github.io/measuring-manuscripts-project-template/). Copy it, make it yours, and put it online. The steps are in *Publish your page* below.
```
...
*The bus factor*. "Bus factor" is a blunt question: how many people would have to be hit by a bus before the project becomes unmaintainable? For most scholarly software the answer is one. One person knows how it was set up, where the data live, which undocumented step makes it work. When they move on, the knowledge leaves with them and the project quietly becomes unfixable.

**Formats that become unreadable.** Data saved in a proprietary or obscure format depend on a particular program to open them. When that program is discontinued or changes, the file becomes a brick. Plenty of 1990s research data now sit in formats nobody can open without effort.
...
Two habits get you most of the way. First, *publish static outputs*: the actual figure, the actual table, saved as files, so the result survives even if the code that made it later breaks. Second, *document the steps*: say where the data came from and how to run the analysis, so the result can be regenerated, not just trusted. Your analysis notebook from the earlier days is itself the documented procedure. Anyone can open it in Colab, run it top to bottom, and the same inputs produce the same figure every time. That is reproducibility at the scale of a course project: a notebook anyone can rerun, the static figure and data it produces, and a README that ties them together.

Use your own running project as the test. By Day 10 you should be able to hand a stranger your repository and have them rebuild your result from the README alone. The template in **project-template/** gives you the scaffolding to do it.
...
You start from the template in **project-template/**, a tiny static site that already works. Your job is to make it yours and put it online. The whole loop is short.

### Make it yours

1. Copy the **project-template/** folder out on its own. It already holds a working page: `index.html` (the text), `style.css` (the look), `figure.png` (your main image), `data.csv` (your data), a `README`, and a `LICENSE`.
2. Open `index.html` in any editor and change the title, your name, the abstract, and the table.
3. Replace `figure.png` with the figure from your own project, and `data.csv` with your data.
4. Edit the `README` to say what your project is and how to rebuild it.

### Put it online with GitHub Pages

1. Make a free account at github.com.
2. Make a new **public** repository. Any name works, but `YOUR-USERNAME.github.io` is the simplest for a personal site.
3. Add your files. The no-terminal way: **Add file → Upload files** on the repository page, drag them in, and commit. The git way: clone the repository, copy the files in, then run `git add .`, `git commit -m "first version"`, and `git push`.
4. Open **Settings → Pages**. Under **Build and deployment**, set **Source** to **Deploy from a branch**, choose the **main** branch and the **/ (root)** folder, and click **Save**.
5. Wait a minute, then visit `https://YOUR-USERNAME.github.io/REPO-NAME/`. From now on, every push updates the live site on its own.

That is the whole skill. A folder of files, a repository, and Pages turned on. No server, no database, nothing to renew.

### Discussion questions

- The same files that make your site are the files you would back up, email, or archive. Why does that make a static site so much easier to preserve than a database behind a server?
- GitHub Pages is free because it serves only static files. What does that stop you from doing, and is that limit a real problem for a research project?
- Your live URL updates the moment you push. What is the risk in that, and how would you keep a fixed version to cite?
