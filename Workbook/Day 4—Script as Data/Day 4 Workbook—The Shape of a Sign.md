...
- A **grapheme** is the abstract letter—the idea of "a," the thing all five marks have in common. It has no single shape; it is the category.
...
- An **allograph** is a recognized *variant form* of a grapheme. Lowercase "a" and the older single-storey "ɑ" are two allographs of the same letter. Roman and italic are allographs. A scribe may use one allograph at the start of a word and another in the middle.
...
The instructor's **Isut Guesser** is one of those direct tools. You draw a sign and it returns the signs most similar to yours. Under the hood that is nearest-neighbors-on-shapes: your drawing becomes a feature vector, every sign in the collection is already a feature vector, and it ranks them by similarity and hands back the closest. It does not need to "read" anything or transcribe a page. Today's notebook is a teaching-scale version of the same idea, built on made-up signs so the whole pipeline runs end to end before real images go in.
